...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/20 14:17:33 by bnoufel           #+#    #+#             */
/*   Updated: 2020/02/20 14:17:34 by bnoufel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check_and_send2(char *str, t_context *ctx, char **cmd)
{
	...
	ft_strdel(&str);
	if ((recv(ctx->socket, check, ft_strlen(ACCEPT), 0)) == -1)
		return (error_handling(451));
...
static size_t	read_file(char **cmd, t_context *ctx, int fd, char *builtin)
{
	t_stat		st;
	char		check[512];
	int			res;
...
int				send_put(char **cmd, t_context *ctx, char *builtin)
{
	...
	if ((fd = open(cmd[1], O_RDONLY)) == -1)
		return (error_handling(550));
	ret = (read_file(cmd, ctx, fd, builtin));
	close(fd);
	return (ret);
}
